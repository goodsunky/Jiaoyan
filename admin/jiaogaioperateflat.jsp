...
<%
	ArrayList<Jiaogai_Info> jislist=(ArrayList<Jiaogai_Info>)session.getAttribute("jislist");
	Jiaogai_Info jgsearch=(Jiaogai_Info)session.getAttribute("jgsearch");
...
function delSel()
{
	...
	{	
		...
		if(confirm('确定要删除选中的吗？'))
			window.location.href="../JiaogaiServlet?action=delete&idarray="+msg;
		else
			return false;
...
function checkdata()
{	
	if((form.startlxsjyear.value!=""&&form.endlxsjyear.value=="")||(form.startlxsjyear.value==""&&form.endlxsjyear.value!=""))
	{
		alert('请选择正确的时间段！');
		return false;
	}else
		if((form.startjtsjyear.value!=""&&form.endjtsjyear.value=="")||(form.startjtsjyear.value==""&&form.endjtsjyear.value!=""))
		{
			alert('请选择正确的时间段！');
			return false;
		}
...
<div id="container">
<div id="content">
<div class="box2"  panelTitle="教改课题信息查询与修改">
<form name="form" method="post" action="../JiaogaiServlet?action=search&pageNo=1" onSubmit="return checkdata()">
<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0>
	<tr>
	<td width="30%" align="right" nowrap="nowrap">课题名称：</td>
    <td width="24%" nowrap="nowrap"><input name="ktmc" type="text" size="20" value="<%=jgsearch!=null&&jgsearch.getJg_ktmc()!=null?jgsearch.getJg_ktmc():"" %>"></td>
    <td width="6%" nowrap="nowrap"><div align="right">负责人或成员：</div></td>
    <td colspan="2" nowrap="nowrap"><input name="fuzeren" type="text" size="4" value="<%=jgsearch!=null&&jgsearch.getJg_fuzeren()!=null?jgsearch.getJg_fuzeren():"" %>"></td>
   	</tr>
   	<tr>
    <td width="18%" nowrap="nowrap"><div align="right">所在部门：</div></td>
    <td width="24%" nowrap="nowrap"><select name="dept">
    ...
    for (Iterator<Department_Info> it = dis.iterator(); it.hasNext();)
	{
		...
		{		
    %>
      <option value="<%=di.getDept_name() %>" <%=jgsearch!=null&&jgsearch.getJg_dept_name()!=null&&jgsearch.getJg_dept_name().equals(di.getDept_name())?"selected":"" %>><%=di.getDept_name() %></option>
      <%}
    }
    %>
    </select></td>
		...
		<select name="ktlb">
          ...
          <%
			    ...
			    for (Iterator<Jiaogaijifen_Info> it = jgjfis.iterator(); it.hasNext();)
				{
					...
					{
			    %>
          <option value="<%=jgjfi.getJgjf_khzb() %>" <%=jgsearch!=null&&jgsearch.getJg_ktlb_name()!=null&&jgsearch.getJg_ktlb_name().equals(jgjfi.getJgjf_khzb())?"selected":"" %>><%=jgjfi.getJgjf_khzb() %></option>
          		<%}
			    }
			    %>
        </select>		</td>
    ...
    <option value="">所有</option>
          <%
          		int startlxsjyear=0;
          		if(jgsearch!=null&&jgsearch.getJg_startlxsj()!=null&&!jgsearch.getJg_startlxsj().equals(""))
          			startlxsjyear=Integer.parseInt(jgsearch.getJg_startlxsj());
    ...
    <option value="">所有</option>
          <%
          		int endlxsjyear=0;
          		if(jgsearch!=null&&jgsearch.getJg_endlxsj()!=null&&!jgsearch.getJg_endlxsj().equals(""))
          			endlxsjyear=Integer.parseInt(jgsearch.getJg_endlxsj());
	...
	年    </td>
		    ...
		    <select name="startjtsjyear" id="startjtsjyear" >
		         <option value="">所有</option>
		          <%
		         	 int startjtsjyear=0;
          			if(jgsearch!=null&&jgsearch.getJg_startjtsj()!=null&&!jgsearch.getJg_startjtsj().equals(""))
          				startjtsjyear=Integer.parseInt(jgsearch.getJg_startjtsj());
		...
		年到<select name="endjtsjyear" id="endjtsjyear">
		         <option value="">所有</option>
		          <%
		           int endjtsjyear=0;
          			if(jgsearch!=null&&jgsearch.getJg_endjtsj()!=null&&!jgsearch.getJg_endjtsj().equals(""))
          				endjtsjyear=Integer.parseInt(jgsearch.getJg_endjtsj());
				    	for(int y=1990;y<2100;y++)
				    	{
				    %>
...
<tr>
    ...
    <td width="24%" nowrap="nowrap">
     <input type="radio" name="isjt" id="isjt" value="all" <%=jgsearch!=null&&jgsearch.getJg_isjt()!=null&&jgsearch.getJg_isjt().equals("all")?"checked":"" %> onClick="ablejtsj()">
     	所有				
     <input type="radio" name="isjt" id="isjt" value="yesjt" <%=jgsearch!=null&&jgsearch.getJg_isjt()!=null&&jgsearch.getJg_isjt().equals("yesjt")?"checked":"" %> onClick="ablejtsj()">
     	已结
     <input type="radio" name="isjt" id="isjt" value="nojt" <%=jgsearch!=null&&jgsearch.getJg_isjt()!=null&&jgsearch.getJg_isjt().equals("nojt")?"checked":"" %> onClick="disablejtsj()">
     	未结	</td>
    <td width="6%"><div align="right">档案号：</div></td>
    <td width="18%"><input type="text" name="danganhao" value="<%=jgsearch!=null&&jgsearch.getJg_danganhao()!=null?jgsearch.getJg_danganhao():"" %>"></td>
    <td width="34%">&nbsp;&nbsp;<input name="save" type="submit" value="查询" class="button"></td>
   </tr>   	
</TABLE>		
</form>
</div>
...
%>
<div>
	<form name="exportform" method="post" action="../JiaogaiServlet?action=export">
						...
						<td>
							<span class="nobreak"> 当前第<%=pageNo%>页 <span
								class="jive-paginator"> [<a
									href="../JiaogaiServlet?action=pagesearch&pageNo=<%=pageNo-1 %>">上一页</a> | <%
								for (int i = 1; i <= totalpages; i++) {
							%> <a href="../JiaogaiServlet?action=pagesearch&pageNo=<%=i %>"> 
							<%
							if(i==pageNo)
							{
							%>
						 ...
						 %> | <a href="../JiaogaiServlet?action=pagesearch&pageNo=<%=pageNo+1 %>">下一页</a>] </span>
								...
								<div class="jive-table">
									<table summary="List of threads" cellpadding="0" cellspacing="0"  WIDTH=100% >
										...
										<%
												...
												for (Iterator<Jiaogai_Info> it = jislist.iterator(); it.hasNext();)
												 {
											...
											<tr class="<%=classStr%>">
												<td width="1%" >
													<div class="render" align="center">
														<input type="checkbox"  value="<%=ji.getJg_id() %>"/> 														
													</div>
												</td>	
												<td width="1%" >
													<div align="center">
												...
												<td width="1%" >
													<div align="center">
													<a href="../JiaogaiServlet?action=showotherinfo&id=<%=ji.getJg_id() %>" onClick="return hs.htmlExpand(this, { objectType: 'iframe' } )">	
													more...
													</a>													
													</div>												
										      </td>	
										      <td width="2%" nowrap="nowrap">
													<div align="center">	
														<a href="../JiaogaiServlet?action=delete&idarray=<%=ji.getJg_id() %>" onclick="return confirm('确定要删除吗？')"><img border="0" alt="删除" src="../icons/delete.gif"/></a>													
														<a href="../JiaogaiServlet?action=alter&id=<%=ji.getJg_id() %>"><img border="0" alt="修改" src="../icons/edit.gif"/></a>
														<a href="../JiaogaiServlet?action=assign&id=<%=ji.getJg_id() %>"><img border="0" alt="积分分配" src="../icons/star.gif"/></a>
													</div>
											  </td>										
											</tr>	
											<%
