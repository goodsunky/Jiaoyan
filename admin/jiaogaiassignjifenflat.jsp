...
<script type="text/javascript" src="../js/checkjiaogaijifenform.js" charset="gb2312"></script>
...
<div id="container">
<div id="content">
<div class="box2" panelTitle="教改课题积分分配">&nbsp;<br>
	<form name="form" method="post" action="../JiaogaiServlet?action=doassign&&id=<%=id %>" onSubmit="return checkdata(<%=lxmcount %>,<%=jtmcount %>,<%=ji.getJg_jxzf() %>)">
	...
	<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0>
		...
		<td width="18%" nowrap="nowrap">
		    <div align="left">
		         【课题类别】<%=ji.getJg_ktlb_name() %>
		   </div>	   
	    </td>
	    <td width="18%" nowrap="nowrap">
		    <div align="left">
		         【本课题绩效总分】<%=ji.getJg_jxzf() %>
		   </div>	   
	    </td>
	    <td width="18%" nowrap="nowrap">
		    <div align="left">
		    	<input type="button" value="一键分配" class="button2" onClick="javascript:window.location.href='../JiaogaiServlet?action=onekeyassign&&id=<%=id %>'">
		   </div>	   
	    </td>
	    </tr>
	    </table>
	    ...
	    <br>
	    <%
		   if(ji.getJg_lxsj()!=null&&!"".equals(ji.getJg_lxsj()))
		   {
		 %>
		 <fieldset>
	     <legend style="font-weight: bold; ">立项时积分分配信息</legend>
		 <%
		  		if(ji.getJg_lxmember()!=null&&"".equals(ji.getJg_lxmember()))
		   		{
		%>
				立项时成员信息为空，无法分配积分！
	    <%
	   ...
	   <br>
	    <%
		   if(ji.getJg_jtsj()!=null&&!"".equals(ji.getJg_jtsj()))
		   {
		 %>
		 <fieldset>
	     <legend style="font-weight: bold; ">结题时积分分配信息</legend>
		 <%
		  		if(ji.getJg_jtmember()!=null&&"".equals(ji.getJg_jtmember()))
		   		{
		%>
				结题时成员信息为空，无法分配积分！
	    <%
	   ...
	   %>
	    ...
	    <tr>
		    ...
		    <td width="50%"  nowrap="nowrap">
		    	&nbsp;&nbsp;
				<input name="cancel" type="button" value="取消"  class="button" onclick="javascript:window.location.href='../JiaogaiServlet?action=cancel';">			    	    
		    </td>
	    </tr>
	    <tr>
		    <td colspan="2">&nbsp;		    </td>
