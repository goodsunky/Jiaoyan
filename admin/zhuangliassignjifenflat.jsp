...
<script type="text/javascript" src="../js/checkzhuanglijifenform.js" charset="gb2312"></script>
...
<div id="container">
<div id="content">
<div class="box2" panelTitle="专利积分分配" panelWidth="100%">
	<br>
	<form name="form" method="post" action="../ZhuangliServlet?action=doassign&id=<%=id %>" onSubmit="return checkdata(<%=mcount %>,<%=zi.getZhl_jxzf() %>)">
	...
	<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0>
	    ...
	    <td width="18%" nowrap="nowrap">
		    <div align="left">
		         【承担单位排名】<%=zi.getZhl_dwpm() %>
		   </div>	   
	    </td>
	    <td width="18%" nowrap="nowrap">
		    <div align="left">
		         【本专利绩效总分】<%=zi.getZhl_jxzf() %>
		   </div>	   
	    </td>
	    <td width="18%" nowrap="nowrap">
		    <div align="left">
		    	<input type="button" value="一键分配" class="button2" onClick="javascript:window.location.href='../ZhuangliServlet?action=onekeyassign&id=<%=id %>'">
		   </div>	   
	    </td>	
	    </tr>
	    </table>
	  ...
	  <br/>
	    <%
		   if(zi.getZhl_sqr()!=null&&!"".equals(zi.getZhl_sqr()))
		   {
		 %>
		 <fieldset>
	     <legend style="font-weight: bold; ">成员积分分配信息</legend>
		 <%
		  		if(zi.getZhl_member()!=null&&"".equals(zi.getZhl_member()))
		   		{
		%>
				成员信息为空，无法分配积分！
	    <%
