...
<script type="text/javascript" src="<%=path+"/js/jquery/jquery.multiselect.js" %>"></script>
...
<script language=javascript>
	...
	{ 
		...
		var newWin = window.open('about:blank','','');
		var bdhtml=document.getElementById("printtable").innerHTML; 
	...
	function checkdata()
	{
		var valuestr = $("#seldept").multiselect("MyValues");
		...
		form.dept.value=valuestr;
	}
	function changexm()
	{
		var url="<%=path+"/JifenhuizongServlet?action=filter&jylx=" %>"+form.jylx.value+"&url="+form.url.value+"&dept="+$("#seldept").multiselect("MyValues")+"&starttime="+form.starttime.value+"&jyxm="+form.jyxm.value;
...
<div id="container">
<div id="content">
			<div class="box2" panelTitle="教研与科研绩效分汇总" >
			<form name="form" method="post" action="<%=path+"/JifenhuizongServlet?action=huizong&tongjitype=admin" %>" onSubmit="return checkdata()">
			<input type="hidden" name="url" value="admin/huizongflat.jsp" />
			...
			<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0>
			    ...
			    <td width="9%" nowrap="nowrap">
				 ...
				 <td width="8%" nowrap="nowrap"><div align="right">时间*</div></td>
				 <td width="9%" nowrap="nowrap">
			    ...
			    <td width="9%" nowrap="nowrap">
			    	<select id="seldept" multiple="multiple" name="seldept">
				    	<%
				    	for (Iterator<Department_Info> it = dis.iterator(); it.hasNext();)
						{
							...
							{	
					    %>
					        <option value="<%=di.getDept_name() %>" <%=null!=dept&&null!=di.getDept_name()&&dept.contains(di.getDept_name())?"selected":"" %>><%=di.getDept_name() %></option>
					        <%}
					    }
					    %>
				    </select>    </td>			    
			...
			<%
				...
				{
			 %>			
			 <form name="exportform" method="post" action="<%=path+"/JifenhuizongServlet?action=export&exptype=admin" %>">
			 <TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0>
			 	<tr>
			 	<td>&nbsp;</td>
			 	</tr>
			...
			<div id="printArea">
			<table class="jive-table" cellspacing="0" cellpadding="0"	bgcolor="#FFFFFF" border="1" align="center">
					<thead>
											<tr>																															
