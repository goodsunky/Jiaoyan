...
<div id="container">
<div id="content">
<div class="box2" panelTitle="论文、论著、标准或画册信息录入（带*号必填）">
	<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0>
		<tr>
		  <td width="5%">
			&nbsp;&nbsp;&nbsp;&nbsp;<a  href="exportfile.jsp?type=LunwenToMysql"  target="_blank" onClick="return hs.htmlExpand(this, { objectType: 'iframe' } )"><img alt="导入数据" border="0" src="../images/import.jpg"></a>       
	      </td>
	      <td align="left">
	      	&nbsp;&nbsp;&nbsp;&nbsp;<a href="../upload/moban/lunwenmoban.xls" target="_blank"><u>模版</u></a>
	      </td>
		</tr>
	</table>
	<br>
	<form name="form" method="post" action="../LunwenServlet?action=add" onSubmit="return checkdata()">
	<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0>
		<tr>		
	    ...
	    <td nowrap="nowrap"><input name="dyzz" type="text" id="dyzz" size="10"></td>
		...
		<td nowrap="nowrap"><select name="dwpm">
	     <option value="">请选择</option>
          ...
          <%
		    ...
		    %>
		  <option value="<%="第"+max %>" ><%="第"+max+"及以后" %></option>
        </select>
        </td>
		<td width="6%"  nowrap="nowrap">
			<div align="right">名称*：</div>		</td>
		<td width="12%" nowrap="nowrap" align="left">
			<input name="lwmc" type="text" id="lwmc" size="22" >			</td>
		<td width="6%"  nowrap="nowrap">
			<div align="right">刊物或学术会议名称：</div>		</td>
		<td width="12%" nowrap="nowrap" align="left">
			<input name="kwmc" type="text" id="kwmc" size="22" >			</td>
...
<select name="cbsjmonth">
 <option value="">请选择</option>
  <%
		    ...
		    %>
  <option value="<%=cbsjmon %>" ><%=cbsjmon %></option>
  <%
		    }
		    %>
</select>
	   ...
	   <select name="lwlb">
          ...
          <%
			    for (Iterator<Lunwenjifen_Info> it = lwlbjfis.iterator(); it.hasNext();)
				{
					...
					{
			    %>
          <option value="<%=kcjfi.getLwjf_xmlb() %>" ><%=kcjfi.getLwjf_xmlb() %></option>
          		<%
			    }}
			    %>
        </select>	
	    ...
	    <td width="17%"  nowrap="nowrap">
          ...
          <%
			    for (Iterator<Lunwenjifen_Info> it = lwjfis.iterator(); it.hasNext();)
				{
					...
					{
			    %>
          <option value="<%=kcjfi.getLwjf_khzb() %>" ><%=kcjfi.getLwjf_khzb() %></option>
          		<%
			    }}
			    %>
        </select>
