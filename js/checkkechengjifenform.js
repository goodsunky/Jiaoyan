function checkdata(lxcount,zjcount,jtcount,zjf)
{
	var lxzjf=document.getElementById("lxzongjifen");
	var zjzjf=document.getElementById("zjzongjifen");
	...
	{
		assjf = 0.0;
		var zjok=validate(zjzjf);	
		if(form.zjzongjifen.value=="")
		{
			alert("中检时的总绩效分不能为空！");
			form.zjzongjifen.select();
			return false;
		}
		if(form.zjzongjifen.value!=""&&zjok=="no")
		{
				alert("中检时的总绩效分必须为非零数字！");
				form.zjzongjifen.select();
				return false;
		}
		asszjf = asszjf +parseFloat(zjzjf.value);
		for(var i=1;i<=zjcount;i++)
		{
			var zjjifen=document.getElementById("zjjifen"+i);
			var ok=validate(zjjifen);
			if(zjjifen.value=="")
			{
				alert("成员积分不能为空！");
				zjjifen.select();
				...
				if(ok=="no")
				{
					...
					{
						assjf = assjf + parseFloat(zjjifen.value);
					}
		}
		if(assjf.toFixed(2)!=parseFloat(zjzjf.value).toFixed(2))
		{
			alert("中检时的绩效分分配不正确！");
			return false;
		}
	...
	if(lxzjf!=null&&zjzjf!=null&&jtzjf!=null&&asszjf.toFixed(2)!=parseFloat(zjf).toFixed(2))
	{
		alert("立项时绩效总分+中检时绩效总分+结题时绩效总分不等于绩效总分！");
		return false;
	}
}
...
function checkzjf(type)
{
  ...
  var url="../KechengServlet?id="+id+"&action=autoassign";
  if(lxzjf!=null&&type=="1")
  {
     ...
     url = url +"&autolxzongjifen="+lxzjf.value;
	  ...
	  if(zjzjf!=null&&type=="2")
	  {
		 var ok=validate(zjzjf);
	     if(zjzjf.value=="")
	     {
	       alert("请输入中检时的绩效总分！");
	       zjzjf.select();
	     ...
	     url = url +"&autozjzongjifen="+zjzjf.value;
	  }else
